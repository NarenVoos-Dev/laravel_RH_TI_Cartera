    ...
    <tbody>
        @foreach ($movimientos as $index => $mov)
            <tr style="background-color: {{ $index % 2 == 0 ? '#ffffff' : '#f9f9f9' }};">
                ...
                <td style="border: 1px solid #000; padding: 6px;">{{ $mov->wallet->employee->name ?? 'No asignado' }}</td>
                <td style="border: 1px solid #000; padding: 6px;">{{ $mov->wallet->company->name ?? 'No asignado' }}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: right;">${{ number_format($mov->amount, 0, ',', '.') }}</td>
                <td style="border: 1px solid #000; padding: 6px; text-align: center;">{{ $mov->created_at->format('Y-m-d') }}</td>
            </tr>
        @endforeach
    </tbody>
</table>
